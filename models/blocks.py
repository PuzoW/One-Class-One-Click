...
def n_global_average(x, batch_lengths):
    """
    Block performing a global average over batch pooling
    :param x: [N, D] input features
    :param batch_lengths: [B] list of batch lengths
    ...
    for b_i, length in enumerate(batch_lengths):
        ...
        # Average features for each batch cloud
        averaged_features.append(torch.sum(x[i0:i0 + length], dim=0)
                                 /(1+torch.sum(torch.sigmoid(x[i0:i0 + length]), dim=0)))
...
class nGlobalAverageBlock(nn.Module):

    def __init__(self):
        """
        Initialize a global average block with its ReLU and BatchNorm.
        """
        super(nGlobalAverageBlock, self).__init__()
        return

    def forward(self, x, batch, layer):
        return n_global_average(x, batch.lengths[layer])
...
class BatchMaxPoolBlock(nn.Module):

    def __init__(self):
        """
        Initialize a max pooling block with its ReLU and BatchNorm.
        """
        super(BatchMaxPoolBlock, self).__init__()
        return

    def forward(self, x, batch, in_layer):
        i0 = 0
        for b_i, length in enumerate(batch.lengths[in_layer]):
            max_p = torch.max(x[i0:i0 + length], dim=0, keepdim=True)[0]

            if b_i == 0:
                maxp_features = max_p
            else:
                maxp_features = torch.cat((maxp_features, max_p), 0)

            i0 += length

        return maxp_features