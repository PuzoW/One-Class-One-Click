...
class ParisDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True):
        ...
        # Dataset feature format
        self.fea_names = ['reflectance']
        ...
        if self.set == 'training':
            self.cloud_names = ['Lille1_1', 'Lille1_2', 'Paris']
            self.files = [os.path.join(self.path, 'training_9_classes', n + '.ply') for n in self.cloud_names]

        elif self.set in ['validation']:
            self.cloud_names = ['Lille2']
            self.files = [os.path.join(self.path, 'training_9_classes', n + '.ply') for n in self.cloud_names]
        
        else:
            self.cloud_names = ['ajaccio_2', 'ajaccio_57', 'dijon_9']
            self.files = [os.path.join(self.path, 'test_10_classes', n + '.ply') for n in self.cloud_names]
    ...
    def batch_item(self, batch_i):
        ...
        for i in range(len(self.batches[batch_i])):
            ...
            if self.config.weak_level is None:
                wl_masks = np.ones(input_points.shape[0], dtype=np.float32)
            else:
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError('Only accepted input dimensions are 1 and 2 (without and with height)')
    ...
    def continuous_block_item(self, batch_i):
        ...
        # Indices of points in input region                                               r=self.config.in_radius)[0]
        ...
        # No data augmentation when validation or testing 
        # input_points, scale, R = self.augmentation_transform(input_points)
    ...
    def potential_item(self, batch_i):
        ...
        while True:
            ...
            if batch_n > int(self.batch_limit) and batch_n > n:
                break
            ...
            # Get original height as additional feature
            input_features = input_colors.astype(np.float32)
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError('Only accepted input dimensions are 1 and 2 (without and with XYZ)')
    ...
    def prepare_ply(self):
        ...
        ply_path = join(self.path, 'training_9_classes')
        if not exists(ply_path):
            makedirs(ply_path)

        file_names = glob.glob(join(self.path, 'training_10_classes', '*.ply'))
        for f in file_names:
            cloud_name = f.split('/')[-1]
            out_file = join(ply_path, cloud_name) 
            if exists(out_file):
                continue
            ...
            valid_inds = np.where(labels != 0)[0]
            points = points[valid_inds]
            reflectance = reflectance[valid_inds]
            labels = labels[valid_inds] - 1

            # Save as ply
            write_ply(out_file, [points, reflectance, labels], ['x', 'y', 'z', 'reflectance', 'class'])
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                sub_colors = data['reflectance'][:, None]
...
def ParisCollate(batch_data):
    return ParisCustomBatch(batch_data[0])