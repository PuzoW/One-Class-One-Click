...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling Hessigheim3D dataset.
...
class H3DDataset(PointCloudDataset):
    """Class to handle H3D dataset."""

    def __init__(self, config, set='training', use_potentials=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'H3D')
        ...
        # Dataset folder
        self.path = 'Data/Hessigheim_Benchmark/Epoch_March18/LiDAR'
        ...
        if self.set == 'training':
            self.cloud_names = ['PC_Mar18_train']
        elif self.set in ['validation']:
            self.cloud_names = ['PC_Mar18_val']
        else:
            self.cloud_names = ['PC_Mar18_test']
        self.files = [os.path.join(self.path, n + '.las') for n in self.cloud_names]
        ...
        if self.config.test_mode:
            ...
            raise ValueError('Unknown set for H3D data: ', self.set)
    ...
    def continuous_block_item(self, batch_i):
        ...
        input_points[:, 2] -= np.min(input_points[:, 2])
        ...
        # No data augmentation when validation and testing
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError('Only accepted input dimensions are 1, 3 and 6 (without and with XYZ)')
    ...
    def potential_item(self, batch_i):
        ...
        while True:
            ...
            if n>self.batch_limit:
                heights = points[input_inds, 2].astype(np.float32)
                sorted_inds = np.argsort(-heights)
                input_inds = input_inds[sorted_inds[:self.batch_limit]]
                n = self.batch_limit

            # Collect labels and colors
            input_points = points[input_inds].astype(np.float32)
            input_points[:, :2] -= center_point[0, 0:2]
            input_points[:, 2] -= np.min(input_points[:, 2])
            input_colors = self.input_colors[cloud_ind][input_inds]
            input_masks = self.input_masks[cloud_ind][input_inds]
            if self.set in ['test']:
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError('Only accepted input dimensions are 1, 4 and 6 (without and with XYZ)')
    ...
    def batch_item(self, batch_i):
        ...
        for i in range(len(self.batches[batch_i])):
            ...
            if self.set in ['test']:
                ...
                input_labels = self.input_labels[cloud_ind][input_inds]
                # if self.set == 'training':
                #     input_labels *= wl_masks.astype(np.int32)
        ...
        if self.config.in_features_dim == 1:
            ...
            raise ValueError('Only accepted input dimensions are 1 (without and with XYZ)')
    ...
    def load_subsampled_clouds(self):
        ...
        if self.set == 'training' and self.config.al_initnum>0:
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                # Read ply file
                points, colors, labels = read_las(file_path)
            ...
            self.input_masks += [np.ones(len(sub_labels), dtype=np.float32)]
        ...
        if self.use_potentials and self.set == 'training':
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(coarse_KDTree_file):
                    ...
                    # write coarse ply file
                    # coarse_ply_file = join(self.tree_path, '{:s}_coarse.ply'.format(cloud_name))
                    # write_ply(coarse_ply_file,
                    #           coarse_points,
                    #           ['x', 'y', 'z'])
                ...
                self.pot_base_prob += [np.zeros((len(coarse_search_tree.data), self.num_classes), dtype=np.float32)]


                # store pot neighbors to speed up subclouds generation
                coarse_neighbors_file = join(self.tree_path, '{:s}_coarse_neighbors_{:.2f}.pkl'.format(cloud_name, self.config.in_radius))
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(proj_file):
                    ...
                    points, _, labels = read_las(file_path)
                    points[:, :2] -= np.mean(points[:, :2], axis=0)
                    labels = labels.astype(np.int32)
    ...
    def load_evaluation_points(self, file_path):
        ...
        # Get original points
        data, _, = read_las(file_path, label=False)
        return data
...
class H3DSampler(Sampler):
    """Sampler for H3D"""

    def __init__(self, dataset: H3DDataset):
...
class H3DCustomBatch:
    """Custom batch definition with memory pinning for H3D"""
...
def H3DCollate(batch_data):
    return H3DCustomBatch(batch_data[0])