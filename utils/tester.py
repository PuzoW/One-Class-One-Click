...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config):
        ...
        self.test_times = [np.zeros(l.shape[0]) for l in test_loader.dataset.input_labels]

        # Test saving path
        if config.saving:
            test_path = join('test', test_loader.dataset.name, config.saving_path.split('/')[-1])
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                if len(batch.cloud_inds.cpu().numpy()) == 0:
                    continue
                ...
                for b_i, length in enumerate(lengths):
                    ...
                    self.test_probs[c_i][inds] += probs
                    self.test_times[c_i][inds] += 1.0
                    i0 += length
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = 'i{:04d} => {:.1f}% (timings : {:4.2f} {:4.2f} {:4.2f})'
                    print(message.format(i,
                                        100 * i / test_loader.dataset.epoch_n,
                                        1000 * (mean_dt[0]),
                                        1000 * (mean_dt[1]),
                                        1000 * (mean_dt[2])))
            ...
            for i, file_path in enumerate(test_loader.dataset.files):
                self.test_probs[i] /= self.test_times[i][:, None]
                ...
                xyz = np.array(test_loader.dataset.input_trees[i].data)

                cloud_name = file_path.split('/')[-1][:-4]
                test_name2 = join(test_path, 'predictions', 'sub_' + cloud_name)
            ...
            for i, file_path in enumerate(test_loader.dataset.files):
                ...
                self.test_probs[i] /= np.sum(self.test_probs[i], axis=1, keepdims=True)
                self.test_probs[i] = np.nan_to_num(self.test_probs[i])
                self.test_probs[i][self.test_probs[i]>1.0]=1.0
            ...
            for i, file_path in enumerate(test_loader.dataset.files):
                ...
                gt = test_loader.dataset.validation_labels[i].astype(np.int32)
                write_ply(test_name,
                            [points, preds, gt],
                            ['x', 'y', 'z', 'preds', 'gt'])
                ...
                pots = self.test_times[i].astype(np.float32)
                write_ply(pot_name,
                            [np.array(test_loader.dataset.input_trees[i].data).astype(np.float32), pots],
                            ['x', 'y', 'z', 'pots'])
                ...
                if test_loader.dataset.set == 'test':
                    if test_loader.dataset.name.startswith('Semantic3D'):
                        ...
                        ascii_name = join(test_path, 'predictions', cloud_name + '.txt')
                    preds += 1
